...
const Header = () => {
    ...
    <header className={`py-5 px-[36px] md:px-[60px] text-[#ffffffb3] font-bold flex justify-between items-center fixed w-full transition-all duration-500 ${isScrolled ? 'bg-[#0f0f0ffa]' : ''} select-none`}>
      ...
      <aside className={`absolute inset-0 h-screen  bg-[#000000b3] transition-all duration-200  ${navIsOpen ? 'opacity-100 z-10' : 'opacity-0 -z-10'}`}>
          <div className={`py-5 px-[36px] md:px-[60px]  h-screen w-fit bg-[#0f0f0f] transition-all duration-200  ${navIsOpen ? 'translate-x-[0%]' : 'translate-x-[-10%] '}`}>
      ...
      <Image src={Logo} alt='Logo HBO Max' className='h-4 w-24 md:w-[134px] md:h-[23px]' />

      <div className='flex items-center gap-3'>
        <BiSearch className='w-6 h-6' />

        <div className='flex items-center'>
          <div className='w-7 h-7 md:w-[38px] md:h-[38px]  rounded-full bg-profile font-semibold relative '>
          ...
          <div className='hidden md:block ml-2 hover:font-bold hover:text-white'>Martín</div>
        </div>
      </div>
    </header>
  );
