...
const Search = () => {
  ...
  const searchMovies = async (e) => {
    ...
    const res = await fetch(
      `https://api.themoviedb.org/3/search/movie?api_key=${process.env.NEXT_PUBLIC_API_KEY}&query=${query}&language=en-US&page=1&include_adult=false`
    );
        ...
        <form className='h-fit' action='' onSubmit={searchMovies}>
          <div className='relative'>
            ...
            <input
              ...
              type='text'
              className=' pl-14 rounded-md placeholder-[#ffffffb3] text-white font-normal text-lg  h-16 border-none outline-none w-full bg-[#3f3d3d50]'
              value={query}
