...
const IndividualMovie = ({ movies, movie, genero, titulo, simil, videos, tvshow }) => {
  ...
  const handleChange = async (e) => {
    ...
    const episodes = await fetch(`https://api.themoviedb.org/3/tv/${movie.id}/season/${e.target.value}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`);
    const data = await episodes.json();
    setSeason(data);
  };

  const fetchSeason = async () => {
    const episodes = await fetch(`https://api.themoviedb.org/3/tv/${movie.id}/season/${selected}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`);
    const data = await episodes.json();
    setSeason(data);
  };
  ...
  return (
    <>
      ...
      {genero ? (
        ...
        <>
          <Banner individual={true} movie={movie} show={tvshow} trailer={trailer} />
          <div className='pl-[28px] sm:pl-[36px] md:pl-[48px] lg:pl-[60px] bg-rowGradient mt-[-6px] mb-[50px] group'>
            ...
            {tvshow ? null : <h2 className='font-bold text-white text-xl mb-3 flex items-center'>Películas recomendadas</h2>}
...
export async function getServerSideProps(context) {
  ...
  if (context.query.genero === 'true') {
    const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&with_genres=${id}`);
  ...
  if (context.query.show === 'true') {
    const res = await fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`);
    const data = await res.json();

    const res3 = await fetch(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`);
    ...
    const res2 = await fetch(`https://api.themoviedb.org/3/movie/${id}/similar?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&page=1`);
    const data2 = await res2.json();
    const similar = data2.results;

    const res3 = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US`);
    const data3 = await res3.json();
    const vids = data3.results;
